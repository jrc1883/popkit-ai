...
class ValidationError:
    ...
    def __str__(self):
        ...
        return f"{self.severity}: {location} - {self.message}" if location else f"{self.severity}: {self.message}"
...
class PluginValidator:
    ...
    VALID_LICENSES = {
        'MIT', 'Apache-2.0', 'GPL-3.0', 'GPL-2.0', 'ISC',
        'BSD-2-Clause', 'BSD-3-Clause', 'LGPL-3.0', 'MPL-2.0',
    ...
    def validate_manifest(self):
        ...
        if 'homepage' in manifest:
            if not isinstance(manifest['homepage'], str) or not manifest['homepage'].startswith('https://'):
                self.errors.append(ValidationError(
                    "ERROR",
    ...
    def validate_structure(self):
        ...
        if manifest_path.exists():
            try:
                ...
                for field in ['commands', 'agents', 'skills', 'hooks', 'mcpServers', 'lspServers']:
    ...
    def validate_hooks(self):
        ...
        if 'hooks' not in hooks:
            self.errors.append(ValidationError(
                "ERROR",
                "hooks.json missing 'hooks' root object",
                str(hooks_json)
            ))
            return
        ...
        for event, event_hooks in hooks['hooks'].items():
            if event not in self.VALID_EVENTS:
                self.errors.append(ValidationError(
                    "ERROR",
                    f"Invalid hook event type: {event}",
                    str(hooks_json)
                ))
            ...
            for matcher_obj in event_hooks:
                ...
                for hook_def in matcher_obj.get('hooks', []):
                    if 'type' not in hook_def:
                        self.errors.append(ValidationError(
                            "ERROR",
                            f"Hook definition missing 'type' field",
                            str(hooks_json)
                        ))
                        continue
        ...
        if skills_dir.exists():
            for skill_dir in skills_dir.iterdir():
                ...
                if not skill_file.exists():
                    ...
                    self._validate_markdown_frontmatter(skill_file, ['name', 'description'])
...
def validate_marketplace(marketplace_path: Path, plugins_dir: Path) -> Tuple[bool, List[ValidationError]]:
    ...
    for plugin_entry in marketplace['plugins']:
        ...
        plugin_json_path = plugins_dir / source.lstrip('./') / '.claude-plugin' / 'plugin.json'
        ...
        try:
            ...
            errors.append(ValidationError(
                "ERROR",
                f"Invalid plugin.json at {source}",
                str(marketplace_path)
            ))
            continue
...
def main():
    ...
    parser.add_argument('--all', action='store_true', help='Validate all plugins in directory')
