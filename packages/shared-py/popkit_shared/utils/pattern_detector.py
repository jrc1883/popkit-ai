...
def detect_component_patterns(directory: Path) -> List[DetectedPattern]:
    ...
    atomic_dirs = _find_dirs(
        directory, ["**/atoms", "**/molecules", "**/organisms", "**/templates"]
    )
    ...
    component_with_index = [
        d
        for d in component_dirs
        if (d / "index.ts").exists() or (d / "index.tsx").exists() or (d / "index.js").exists()
    ]
    ...
    flat_components = _find_files(
        directory, ["**/components/*.tsx", "**/components/*.jsx", "**/components/*.vue"]
    )
...
def detect_api_patterns(directory: Path) -> List[DetectedPattern]:
    ...
    if routes_dir:
        route_files = _find_files(
            directory, ["**/routes/*.ts", "**/routes/*.js", "**/routers/*.ts", "**/routers/*.js"]
        )
        if route_files:
            patterns.append(
                DetectedPattern(
...
def detect_state_patterns(directory: Path) -> List[DetectedPattern]:
    ...
    if _has_dependency(pkg, "zustand"):
        zustand_files = _find_files(
            directory, ["**/store*.ts", "**/store*.js", "**/*Store.ts", "**/*Store.js"]
        )
        patterns.append(
            DetectedPattern(
                name="zustand",
    ...
    # Custom hooks pattern
    hook_files = _find_files(
        directory,
        ["**/hooks/use*.ts", "**/hooks/use*.tsx", "**/hooks/use*.js", "**/hooks/use*.jsx"],
    )
    if len(hook_files) >= 3:
        patterns.append(
            DetectedPattern(
...
def detect_naming_conventions(directory: Path) -> List[DetectedPattern]:
    ...
    pascal_components = _find_files(
        directory, ["**/components/**/[A-Z]*.tsx", "**/components/**/[A-Z]*.jsx"]
    )
    kebab_components = _find_files(
        directory, ["**/components/**/[a-z]*-[a-z]*.tsx", "**/components/**/[a-z]*-[a-z]*.jsx"]
    )
    ...
    test_files = _find_files(
        directory, ["**/*.test.ts", "**/*.test.tsx", "**/*.test.js", "**/*.test.jsx"]
    )
    spec_files = _find_files(
        directory, ["**/*.spec.ts", "**/*.spec.tsx", "**/*.spec.js", "**/*.spec.jsx"]
    )
    ...
    index_files = _find_files(
        directory, ["**/index.ts", "**/index.tsx", "**/index.js", "**/index.jsx"]
    )
    ...
    if colocated_test_dirs or len(colocated_tests) >= 3:
        ...
        patterns.append(
            DetectedPattern(
                ...
                category="testing",
                confidence=min(
                    0.5 + 0.1 * (len(colocated_test_dirs) + len(colocated_tests) // 3), 0.9
                ),
                examples=examples[:5],
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--min-confidence", "-m", type=float, default=0.0, help="Minimum confidence threshold"
    )
