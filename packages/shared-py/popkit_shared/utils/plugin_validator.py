...
def validate_agent_config(agent_config: Dict[str, Any], agents_dir: Path) -> Dict[str, Any]:
    ...
    if orphaned:
        ...
        result['warnings'].append(f"Orphaned agent files (not registered): {', '.join(sorted(orphaned))}")

    result['valid'] = result['has_agents_field'] and result['has_routing_field'] and result['agent_files_exist']
    ...
    if hooks_dir.exists():
        ...
        if hooks_json_path.exists():
            try:
                ...
                orphaned['python_files'] = sorted(list(all_py_files - referenced_files - test_files))
