...
def build_timeline_summary(events: List[Dict[str, Any]], reasoning_lookup: Dict[str, Dict[str, Any]]) -> str:
...
def generate_narrative_with_api(session_data: Dict[str, Any], events: List[Dict[str, Any]],
                                reasoning_lookup: Dict[str, Dict[str, Any]],
                                total_tokens: Optional[Dict[str, int]] = None) -> str:
    """
...
def generate_template_narrative(session_data: Dict[str, Any], events: List[Dict[str, Any]],
                                total_tokens: Optional[Dict[str, int]] = None) -> str:
...
The primary tools used were {tools_str}, indicating a focus on code analysis, file operations, and development workflows."""
    ...
    if total_tokens and total_tokens['total_tokens'] > 0:
        ...
        if total_tokens['input_tokens'] + total_tokens['cache_read_input_tokens'] > 0:
            cache_rate = (total_tokens['cache_read_input_tokens'] /
...
def generate_session_narrative(recording_file: Path, events: List[Dict[str, Any]],
                              reasoning_lookup: Dict[str, Dict[str, Any]],
                              total_tokens: Optional[Dict[str, int]] = None) -> str:
    """
    ...
    html_paragraphs = [f'<p style="margin-bottom: 12px; line-height: 1.6;">{p.strip()}</p>'
...
def generate_narrative(recording_file: Path, events: List[Dict[str, Any]],
                      reasoning_lookup: Dict[str, Dict[str, Any]],
                      total_tokens: Optional[Dict[str, int]] = None) -> str:
