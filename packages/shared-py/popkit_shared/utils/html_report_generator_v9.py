#!/usr/bin/env python3
"""
HTML Report Generator V9 for Session Recordings

Enhanced with:
- Fixed table layout (no horizontal overflow)
...
def try_parse_python_structure(text: str) -> Optional[Any]:
    """Try to parse text as Python dict/list using ast.literal_eval."""
    try:
...
def format_code_content(text: str) -> str:
    """Format text content with smart parsing and syntax highlighting."""
    if not text:
        return '<div class="text-preview"><em>empty</em></div>'

    if len(text) < 50:
        return f'<div class="text-preview">{escape_html(text)}</div>'

    # Try to parse as Python structure
    parsed_python = try_parse_python_structure(text)
    if parsed_python is not None:
        pretty = pretty_print_structure(parsed_python)
...
def parse_transcript_for_reasoning(
    ...
    try:
        ...
        for tool_use in tool_uses:
            ...
            lookup[tool_use_id] = {"reasoning": reasoning, "tokens": tokens, "cost": cost}
...
def generate_html_report(recording_file: Path, output_file: Path) -> None:
    ...
    if reasoning_lookup:
        total_input = sum(
            v["tokens"].input_tokens for v in reasoning_lookup.values() if v["tokens"]
        )
        total_output = sum(
            v["tokens"].output_tokens for v in reasoning_lookup.values() if v["tokens"]
        )
        total_cache_write = sum(
            v["tokens"].cache_creation_input_tokens
            for v in reasoning_lookup.values()
            if v["tokens"]
        )
        total_cache_read = sum(
            v["tokens"].cache_read_input_tokens for v in reasoning_lookup.values() if v["tokens"]
        )
        total_tokens = {
    ...
    completion_map = {}  # sequence -> has_completion
    for event in unified_timeline:
        if event.get("type") == "tool_call_start":
            seq = event.get("sequence")
            tool_name = event.get("tool_name")

            # Look for matching completion
            has_completion = False
            for complete_event in unified_timeline:
                if (
                    complete_event.get("type") == "tool_call_complete"
                    and complete_event.get("tool_name") == tool_name
                    and complete_event.get("sequence", -1) > seq
                ):
                    has_completion = True
                    break

            completion_map[seq] = has_completion
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .timeline-scroll {{
            max-height: 1000px;
            overflow-y: auto;
            overflow-x: hidden; /* NO horizontal scroll on outer container */
        }}
        ...
        /* Fixed table layout - prevents cells from expanding */
        .timeline-table {{
            width: 100%;
        ...
        .code-block {{
            ...
            max-height: 400px;
            overflow-y: auto;
            overflow-x: auto; /* Allow horizontal scroll INSIDE code block */
    ...
    for i, event in enumerate(unified_timeline, 1):
        ...
        if event_type == "session_start":
            ...
            status_html = (
                ...
                if has_completion
                else '<span class="status-na">⏳ Running</span>'
            )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print("Usage: python html_report_generator_v9.py <recording.json> <output.html>")
        sys.exit(1)
