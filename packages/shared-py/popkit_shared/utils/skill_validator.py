...
def validate_skill_format(skill_file: Path) -> Dict[str, Any]:
    ...
    if section_count < 2:
        result['warnings'].append(f"Only {section_count} standard sections found (expected at least 2)")

    # Validate skill name format
    if 'name' in result['frontmatter']:
        name = result['frontmatter']['name']
        if not re.match(r'^[a-z0-9-:]+$', name):
            result['errors'].append(f"Invalid skill name format: {name} (should be lowercase with hyphens)")
        ...
        if skill_file.name == 'SKILL.md':
            ...
            if dir_name != expected_name and not dir_name.startswith(expected_name):
                result['warnings'].append(f"Directory name '{dir_name}' doesn't match skill name '{name}'")
...
def check_skill_naming_consistency(skills_dir: Path) -> Dict[str, Any]:
    ...
    for skill_dir in skill_dirs:
        ...
        if 'name' not in validation['frontmatter']:
            result['issues'].append({
                'directory': skill_dir.name,
                'issue': 'Missing name in frontmatter'
            })
            result['inconsistent'] += 1
            continue
