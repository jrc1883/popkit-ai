            ...
            for line in f:
                ...
                try:
                    ...
                    messages.append({
                        ...
                        'content': msg.get('content', []) if isinstance(msg.get('content'), list) else [{'type': 'text', 'text': str(msg.get('content', ''))}]
                    })
...
def generate_html_report(recording_file: Path, output_file: Path) -> None:
    ...
    total_observable = len(sorted_events) + sum(t.get('message_count', 0) for t in subagent_transcripts.values())
