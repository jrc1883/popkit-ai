#!/usr/bin/env python3
"""
HTML Report Generator V7 for Session Recordings

Enhanced with:
- Syntax highlighting for code content (like VS Code)
- Proper formatting with line breaks and indentation
...
def format_code_content(text: str) -> str:
    ...
    return f"""
    <pre class="code-block"><code class="language-{language}">{escaped_text}</code></pre>
    """
...
def format_params_inline(params: Dict[str, Any]) -> str:
    ...
    if "command" in params:
        cmd = params["command"]
        key_params.append(f"<code>{cmd[:80]}...</code>" if len(cmd) > 80 else f"<code>{cmd}</code>")
...
def generate_html_report(recording_file: Path, output_file: Path) -> None:
    """Generate enhanced HTML report with syntax highlighting."""
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print("Usage: python html_report_generator_v7.py <recording.json> <output.html>")
        sys.exit(1)
