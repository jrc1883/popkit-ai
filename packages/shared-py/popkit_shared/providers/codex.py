...
class CodexLaunchSpec:
    """Launch information for a new Codex session."""
...
class CodexAdapter(ProviderAdapter):
    ...
    def detect(self) -> ProviderInfo:
        ...
        return ProviderInfo(
            ...
            interaction_surfaces=(
                InteractionSurface.PLAIN_TEXT,
                InteractionSurface.REQUEST_USER_INPUT,
            ),
            default_interaction_surface=InteractionSurface.PLAIN_TEXT,
        )
    ...
    def generate_config(self, package_dir: Path, output_dir: Path) -> list[Path]:
        ...
        with open(config_path, "w", encoding="utf-8") as f:
            f.write(
                "# Generated by: popkit provider wire codex\n"
                "# POPKIT_PROVIDER identifies Codex sessions for PopKit hooks.\n"
                "# Interaction surfaces such as request_user_input are host-launched"
                " runtime choices, not config-only features.\n\n"
            )
    ...
    def get_launch_metadata(self) -> dict[str, Any]:
        """Return host-launch metadata for Codex sessions."""
        return {
            "provider": "codex",
            ...
            "modes": {
                "default": {
                    "profile": "popkit",
                    "interaction_surface": InteractionSurface.PLAIN_TEXT.value,
                    "config_overrides": [],
                },
                "plan": {
                    "profile": "popkit-plan",
                    "interaction_surface": InteractionSurface.REQUEST_USER_INPUT.value,
    ...
    def build_launch_spec(
        ...
        *,
        ...
        host_plan_supported: bool = False,
    ) -> CodexLaunchSpec:
        """Build a host-owned Codex launch spec."""
        requested_mode = mode if mode in {"default", "plan"} else "default"
        actual_mode = requested_mode
        warning: str | None = None

        launch_env = dict(env or {})
        launch_env["POPKIT_PROVIDER"] = "codex"
        launch_env["POPKIT_CAN_ASK_USER_QUESTION"] = "0"

        if requested_mode == "plan" and host_plan_supported:
            interaction_surface = InteractionSurface.REQUEST_USER_INPUT
            launch_env["POPKIT_INTERACTION_SURFACE"] = interaction_surface.value
            launch_env["POPKIT_CAN_REQUEST_USER_INPUT"] = "1"
        else:
            if requested_mode == "plan":
                actual_mode = "default"
                warning = (
                    "Plan mode was requested, but this host has not declared Codex "
                    "request_user_input support. Falling back to standard launch."
                )
            interaction_surface = InteractionSurface.PLAIN_TEXT
            launch_env["POPKIT_INTERACTION_SURFACE"] = interaction_surface.value
            launch_env["POPKIT_CAN_REQUEST_USER_INPUT"] = "0"
