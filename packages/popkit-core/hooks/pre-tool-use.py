...
class PreToolUseHook:
    ...
    def parse_xml_context(self, conversation_history: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        ...
        Extracts category, severity, and workflow from <problem> tag.
        ...
        Returns:
            Dict with parsed context or None if no XML found:
            {
                ...
                "workflow": str,  # category-specific workflow XML
                "stack": List[str],  # detected tech stack
                "infrastructure": Dict[str, Any]  # detected infrastructure
            }

        Example:
            >>> history = [{"role": "user", "content": "...<problem><category>bug</category>..."}]
