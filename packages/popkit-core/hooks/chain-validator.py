...
class ChainValidator:
    ...
    def validate_workflow(
        ...
        errors = []
        warnings = []
        stats = {"total_phases": 0, "agents_used": set(), "missing_agents": set()}
            ...
            for i, phase in enumerate(phases):
                ...
                for agent in phase_agents:
                    ...
                    if agent not in self.available_agents:
                        warnings.append(
                            f"Phase '{phase_name}': Agent '{agent}' not found in tier definitions"
                        )
    ...
    def get_workflow_visualization(self, workflow_id: str) -> str:
        ...
        if phases:
            ...
            lines.append("")
            lines.append(
                f"Phases: {len(phases)} | Agents: {sum(len(p.get('agents', [])) for p in phases)}"
            )
        ...
        if simple_agents and not phases:
            ...
            lines.append("")
            lines.append(
                f"Agents: {len(simple_agents)} | Mode: {'sequential' if sequential else 'parallel'}"
            )
...
def main():
    ...
    try:
        ...
        if results["summary"]["total_warnings"] > 0:
            print(
                f"Chain validation: {results['summary']['total_warnings']} warnings",
                file=sys.stderr,
            )
