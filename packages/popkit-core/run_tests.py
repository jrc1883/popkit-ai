...
def main():
    ...
    if output_json:
        ...
        for cat, cat_results in sorted(results['categories'].items()):
            ...
            if verbose or cat_results['failed'] > 0:
                for test in cat_results['tests']:
                    ...
                    print(f"  {test_status} {test['name']} ({test['passed']}/{test['passed'] + test['failed']}, {test['duration_ms']}ms)")
        ...
        print(f"Passed: {results['passed']} ({results['passed']/results['total']*100:.1f}%)" if results['total'] > 0 else "Passed: 0 (0%)")
        print(f"Failed: {results['failed']} ({results['failed']/results['total']*100:.1f}%)" if results['total'] > 0 else "Failed: 0 (0%)")
