...
export const indexTemplate = `#!/usr/bin/env node
...
// Workspace path (from environment or cwd)
    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
      tools: [
        // Expose tool_search for dynamic discovery
        {
          name: 'tool_search',
          description:
            'Search for relevant development tools using natural language. Returns matching tools.',
          inputSchema: {
            type: 'object',
            properties: {
              query: {
        ...
        // Also expose all tools directly
          ...
          content: [
            {
              ...
              text: JSON.stringify(
                {
                  message: \`Found \${results.length} relevant tools.\`,
                  tools: results.map((r) => ({
                    name: r.tool.name,
