...
def hybrid_search(
    ...
    semantic_available = (
        semantic_results
        and semantic_results[0].get("status") != "unavailable"
        and semantic_results[0].get("status") != "error"
    )
...
def main():
    ...
    parser.add_argument(
        "--mode",
        choices=["sources", "semantic", "keyword", "hybrid", "status"],
        default="hybrid",
        help="Search mode",
    )
    ...
    if args.mode == "sources":
        ...
        hybrid_result = hybrid_search(
            args.query, content, args.top_k, args.semantic_weight, args.keyword_weight
        )
